...
if (T){
  ...
  for (A in A_vec){
    ...
    #plot(colMeans(y))
    stan_list$y =y 
    run = sampling(model, stan_list, iter = 130, chains=6, control = list(max_treedepth = 18, adapt_delta = 0.995))
  ...
  print(ggplot(errors) + geom_line(aes(A, mse_2)) + labs(title = "MSE of  beta .* one_hot", y="Error", x="A") + 
    scale_x_continuous(breaks= A_vec))
